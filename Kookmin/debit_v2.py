...
def findtableortext(bs): # 테이블 추출
    if(str(bs)[1:6]=="table"):
        return str(bs).replace('\n', '')
    else:
        return bs.text.replace('\n', '')
        ...
        if str(child)[:17] == '<div id="tabCon01':
            for d in child.findAll(recursive=False):
                ...
                if str(d)[1:28]=='div class="benefitBox1 marT': 
                    ...
                    if str(d)[:19]== '<div class="titArea':
                        benefitsentence+=findBenefitminititle(d)
                    else:
                        benefitsentence+=findtableortext(d) 
            ...
            if str(child)[:28] == '<div class="benefitBox1 marT':
                ...
                if str(child)[:19]== '<div class="titArea':
                    benefitsentence+=findBenefitminititle(child)
                else:
