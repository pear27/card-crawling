...
'''
    1. 하나 신용카드 데이터베이스 저장
        python3 ./Hana/DatabaseInsert.py Credit

    2. 하나 체크카드 데이터베이스 저장
        python3 ./Hana/DatabaseInsert.py Debit
'''
...
try:
    with connection.cursor() as cursor:
        # CSV 파일 읽기
        with open(f'./Hana/{csv_file}_benefit.csv', 'r', encoding='utf-8') as csvfile:
        ...
        connection.commit()
        print(f"{current_time} [하나 {insert_type}] --- DB update 완료 ")
