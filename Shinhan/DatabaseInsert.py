...
'''
    1. 신한 신용카드 데이터베이스 저장
        python3 ./Shinhan/DatabaseInsert.py Credit

    2. 신한 체크카드 데이터베이스 저장
        python3 ./Shinhan/DatabaseInsert.py Debit
'''
...
try:
    with connection.cursor() as cursor:
        # CSV 파일 읽기
        with open(f'./Shinhan/{csv_file}_benefit.csv', 'r', encoding='utf-8') as csvfile:
        ...
        connection.commit()
        print(f"{current_time} [신한 {insert_type}] --- DB update 완료 ")
