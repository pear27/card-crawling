...
'''
    1. 롯데 신용카드 데이터베이스 저장
        python3 ./Lotte/DatabaseInsert.py Credit

    2. 롯데 체크카드 데이터베이스 저장
        python3 ./Lotte/DatabaseInsert.py Debit
'''
...
try:
    with connection.cursor() as cursor:
        # CSV 파일 읽기
        with open(f'./Lotte/{csv_file}_benefit.csv', 'r', encoding='utf-8') as csvfile:
        ...
        connection.commit()
        print(f"{current_time} [롯데 {insert_type}] --- DB update 완료 ")
